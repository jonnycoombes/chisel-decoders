...
fn benchmark_canada(c: &mut Criterion) {
    c.bench_function("native decode canada.json file", |b| b.iter(canada));
}

fn benchmark_twitter(c: &mut Criterion) {
    c.bench_function("native decode twitter.json file", |b| b.iter(twitter));
}

fn benchmark_citm_catalog(c: &mut Criterion) {
    c.bench_function("native decode citm_catalog.json file", |b| {
        b.iter(citm_catalog)
    });
}
criterion_group! {
